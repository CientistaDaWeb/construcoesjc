...
class ContasReceber_Model extends WS_Model {
    ...
    public function adjustToView($data) {
        ...
        $data['linkBoleto'] = '<a target="blank" href="/erp/Boleto-Sicredi/boleto/' . base64_encode($data['id']) . '">Imprimir</a>';
        return $data;
    }
    ...
    public function buscarPorPeriodo($dataInicial = '', $dataFinal = '') {
    ...
    public function buscarAcumulado($data_inicial) {
        $consulta = $this->_db->select()
                ->setIntegrityCheck(false)
                ->from('contas_receber', array('soma' => 'SUM(valor_pago)'))
                ->where('data_pagamento < ?', $data_inicial);
        $item = $consulta->query()->fetch();
        return $item;
    }
    ...
    public function buscarPorCliente($cliente_id) {
        $consulta = $this->_db->select()
                ->setIntegrityCheck(false)
                ->from(array('cr' => 'contas_receber'), array('id', 'data_vencimento', 'valor', 'data_pagamento', 'valor_pago', 'orcamento_id'))
                ->joinInner(array('o' => 'orcamentos'), 'o.id = cr.orcamento_id', array(''))
    ...
    public function buscarParaBoleto($boleto_id) {
        $consulta = $this->_db->select()
                ...
                ->joinInner(array('ca' => 'caixas'), 'e.id = ca.empresa_id', array('conta_corrente', 'agencia', 'carteira'))
                ->joinLeft(array('ce' => 'clientes_enderecos'), 'ce.id = cr.endereco_id', array('cliente_endereco' => 'endereco', 'cliente_cidade' => 'cidade', 'cliente_bairro' => 'bairro', 'cliente_numero' => 'numero', 'cliente_cep' => 'cep'))
    ...
    public function getRetainedByPeriod($dataInicial, $dataFinal) {
        $sql = $this->_db->select()
                ->setIntegrityCheck(false)
