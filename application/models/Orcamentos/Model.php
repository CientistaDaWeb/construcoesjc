...
class Orcamentos_Model extends WS_Model {
    ...
    public function find($id) {
        $consulta = $this->_db->select()
                ->setIntegrityCheck(false)
                ->from(array('o' => 'orcamentos'), array('*'))
                ->joinInner(array('e' => 'empresas'), 'e.id = o.empresa_id', array('empresa' => 'razao_social', 'endereco', 'numero', 'cidade', 'complemento', 'bairro', 'cep', 'telefone', 'telefone2', 'telefone3', 'logomarca', 'site', 'email'))
    ...
    public function gerarPdf($id, $timbrado = false) {
        ...
        $file = realpath('../livedocx/Orcamento.docx');
        $phpLiveDocx->setLocalTemplate($file);
        ...
        if (!$phpLiveDocx->imageExists($photoFilename)) :
            $phpLiveDocx->uploadImage('http://www.' . $configs->cliente->dominio . '/uploads/logos/' . $photoFilename);
        endif;
